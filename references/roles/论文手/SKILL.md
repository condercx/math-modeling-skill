...
目标为 2026 第二十三届“华为杯”时，在任何论文构建前先读取 `../../../references/competitions/huawei-cup-2026.md`。

论文目录固定采用：
- `paper/template/官方论文模板.doc`：官方附件 3 原件，只读；
- `paper/working/论文工作稿.doc`：Word 主工作稿；
- `paper/sections/`：每个 qN 的证据草稿，不是最终提交物；
- `paper/checkpoints/`：V1/V2 可回滚版本；
- `paper/export/`：PDF 候选版；
- `submission/locked/`：提交 MD5 后锁定的最终 PDF。

禁止自己重做封皮。首页必须保留附件 3 的原封皮和 4 个 logo；第二页起使用统一摘要页。除首页外不得出现学校、队员姓名、队伍编号等身份信息。摘要必须覆盖建模思路、主要方法、模型、结果与结论、创新点、关键词，且不超过两页。最终上传文件必须为 PDF，并按 `题号+队伍编号.pdf` 命名。

Word 是团队写作主源，PDF 是官方提交物。W2 前必须进行一次**Human Rewrite/Review**：
- 每位队员至少审读自己负责的 qN；
- 摘要、结论和关键模型说明由人重新组织语言；
- 核对所有 AI 辅助数据分析旁注和代码头注释；
- 核对引用程序、网页/博客和文献来源；
- 不允许未理解、未核验的 AI 文本直接进入最终 PDF。

AI 生成且无法确认来源的模型/公式默认从最终稿移除；如团队坚持保留，必须按官方规定标注 AI 工具信息和风险，并保留人工推导/核验记录。
