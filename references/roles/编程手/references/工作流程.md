...
完成数据读取、预处理和核心求解纵向切片后，先从 `PROJECT_ROOT` 执行一条最小真实命令：

1. 每个会驱动后续全量计算的模型族至少覆盖一个真实输入或结构等价小实例。
2. 核对退出码、输入到结果的追溯、单位、数值范围、关键约束和模型合同。
3. 实现问题由编程手直接修正；公式、约束或参数合同问题携命令、堆栈和预期—实际证据返回建模手。

复杂或高风险任务可以在扩大计算前按 `../../../../references/Subagent调度.md` 安排独立只读复核，`P1` 仅作可选标签。复核用于尽早暴露接口问题，不是全量计算、参数扫描或正式出图的审批条件；未安排复核不构成未完成。
...
3. 图表数量按证据和论文需要决定。如建立 `raw_q1_*`、`process_q1_*`、`result_q1_*` 候选池，建议三类都覆盖全部子问题并保留多版比较；每张正式图同时导出 SVG 与 300 DPI PNG。
...
6. 需要解释模型结构时生成建模流程图（`flow_overall_model` / `flow_qN_model`）：按 `tools/figure/references/chart-types/flowchart.md` 执行，子问题存在独立分支、循环或三步以上链时可补子问题图；节点与连线来自真实模型/代码，用已安装的 Matplotlib/MATLAB 生成并保留可运行代码。流程图不与结果图互相替代。
...
代码、结果表、必要图表和复现说明完成后，复杂或高风险任务可以按 `../../../../references/Subagent调度.md` 安排独立只读复核，`P2` 仅作可选标签。复核者可重跑唯一复现命令，核对哈希、环境、种子、参数、关键数值、约束、边界、量纲、统计计算和各子问题图表语义；需要时把带全部 `--questions` 的 `figure_audit.py --strict` 结果和实际 PNG/灰度预览作为证据输入。未安排复核不构成不能进入论文阶段，论文只能使用真实、可追溯结果。
