...
| PROBLEM_SELECTION | A-F 三人初筛、主 Agent 三视角复筛、人工选题 | selection/final-selection.md + DECISIONS.md |
...
每个 qN：`RESEARCH（可按项目路由并行探索） -> MODEL（主 Agent + 人） -> INTERFACE -> SCAFFOLD -> IMPLEMENT -> RUN -> VERIFY -> PAPER -> PASS`。
