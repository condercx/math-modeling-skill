...
6. 运行环境支持 Subagent 且任务会生成或修改固定交付物时，按 `references/Subagent调度.md` 在关键节点立即派发独立质检；禁止等全流程结束后才首次派发，作者自检不能替代独立验收。除固定质检外，通用模式不主动派发其他 Subagent；2026 华为杯团队模式按 `TEAM-SKILL.md` 将 Luna max 的独立高 token 探索作为用户已批准的例外。
...
4. **必须至少 1 幅总体建模流程图**，覆盖输入、子问题依赖、模型、求解、验证与输出；子问题存在独立分支、循环或三步以上求解链时，补充对应子问题流程图。文件名使用 `flow_overall_model.*`、`flow_qN_model.*`，与三类候选图分开计数，不替代结果图。
...
- 所有计算结论来自实际运行结果。
- 公式、表格和图表与代码结果一致。
- 引用须由 OpenAlex、原始出版页面或其他可靠来源追溯；AnySearch 可用时可做交叉核验，但团队模式不强制。
...
当项目为三人研究生数学建模竞赛，且根目录含 `STATUS.md` / `TASKS.md` / `problem/dependency.md`，或用户询问团队进度时，读取本 Skill 根目录 `TEAM-SKILL.md` 并以其团队状态机执行。2026 华为杯团队模式的模型/子代理路由以该文件与工作仓库 `AGENTS.md` 为准：Sol 主线、可用时 Luna max 处理高 token 的独立探索，不再要求 DSH/AnySearch。

团队扩展不会替代本 Skill 的建模手、编程手、论文手和 M1/P1/P2/W1/W2 独立门禁；增加三人 A-F 选题、Q1..QN DAG、证据链、异构执行及进度管理。通用模式的“额外 Subagent 默认关闭”在团队模式下由 `TEAM-SKILL.md` 明确覆盖，只针对限定的探索/整理任务；已参与写作的子代理不得审核同一产物。
