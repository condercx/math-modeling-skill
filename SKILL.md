...
6. 运行环境支持 Subagent 且任务会生成或修改固定交付物时，按 `references/Subagent调度.md` 在关键节点立即派发独立质检；禁止等全流程结束后才首次派发，作者自检不能替代独立验收。除固定质检外，通用模式不主动派发其他 Subagent；团队模式是否允许额外协作及其模型路由，以项目 `AGENTS.md`、项目模型路由文件和用户授权为准。
...
当项目为三人研究生数学建模竞赛，且根目录含 `STATUS.md` / `TASKS.md` / `problem/dependency.md`，或用户询问团队进度时，读取本 Skill 根目录 `TEAM-SKILL.md` 并以其团队状态机执行。若项目另有 `AGENTS.md` 或模型路由文件，项目规则是当前任务的唯一型号与 effort 准据；本 Skill 不把某个模型绑定为会话身份，也不从历史回执推导未来默认路由。

团队扩展不会替代本 Skill 的建模手、编程手、论文手和 M1/P1/P2/W1/W2 独立门禁；增加三人 A-F 选题、Q1..QN DAG、证据链、异构执行及进度管理。额外 Subagent 是否启用由用户授权和项目规则决定；已参与写作或修正的子代理不得审核同一产物。
