## 2026-09-23 — Team workflow migration

- 将 2026 华为杯团队模式改为 GPT-6 Sol 主线，Luna max 可用时优先执行独立高 token 探索/资料整理；不再要求 DSH/AnySearch。历史 DSH 插件与测试保留为兼容资料。
- 同步团队 Skill 的选题、调研、编码、Handoff 和状态机说明，不更改独立质量门禁或赛事合规规则。
...
- 保留复杂模型的按需独立复核，但移除建模、编程和论文深层参考中“未过 M1/P1/P2/W1/W2 不得继续”的强制流程。
- 编程和论文的候选图、流程图、复现说明按真实证据需要组织，不再把固定图数或复核回执作为继续工作的前置条件。
- 团队参考仅保留轻量协作建议；`STATUS/TASKS/DECISIONS/handoff`、Gate 和固定 owner 不再作为默认流程。
